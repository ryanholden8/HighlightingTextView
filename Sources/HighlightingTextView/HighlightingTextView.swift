            ...
            if let charRange = characterRange(at: location) {
                ...
                if let editingHighlight = highlights.first(where: {$0.nsRange.overlaps(charNsRange)}) {
                    ...
                    guard !isSelectedRangeInsideUserTap else {
                        print("Tap detected while using iOS text selection. Aborting highlight menu.")
                        return
                    }
                    ...
                    guard !actions.isEmpty else {
                        print("No actions provided for highlight at range: \(editingHighlight.nsRange). Aborting highlight menu.")
                        return
                    }
